...
def download_loupe_converter(path: str) -> None: 
    '''
    ...
    if OPERATING_SYSTEM.startswith('linux'):
        tmp_path = path + '.tmp'
        md5=_md5_checksum("https://github.com/10XGenomics/loupeR/"
        "releases/download/v1.1.4/louper-linux-x64", tmp_path)
        if md5 != "b3fd93fd88a43fbcf3f6e40af3186eaa":
            raise OSError('MD5 checksum failed, please try again. '
            'If this problem persists, please raise an issue on the github.')
        os.rename(tmp_path, "")
        os.chmod(path, 0o755)
    elif OPERATING_SYSTEM.startswith('win'):
        tmp_path = os.environ['TEMP'] + '/louper.exe'
        md5 = _md5_checksum("https://github.com/10XGenomics/loupeR/releases/"
        "download/v1.1.4/louper-windows-x64.exe", tmp_path)
        ...
        md5 = _md5_checksum("https://github.com/10XGenomics/loupeR/"
        "releases/download/v1.1.4/louper-macos-x64", tmp_path)
        if md5 != "f5d1e99138e840169a19191d10bb25ab":
            raise OSError('MD5 checksum failed, please try again. If this problem persists, '
            'please raise an issue on the github.')
        os.rename(tmp_path, path)
    else:
...
def eula(path: str) -> None:
    ...
    if not os.path.exists(f'{path}/eula/eula_agreement'):
        ...
        "(https://www.10xgenomics.com/legal/terms-of-use/).")
